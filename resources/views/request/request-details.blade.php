		...
		<form method="POST" action="{{ url('/request/save-request-detail') }}/{{$l->id}}"  accept-charset="utf-8" enctype="multipart/form-data">
			...
			<table border="1px #fff solid;" style="table-layout: fixed;">
				<tr>
					...
					<td colspan="2" class="p-3 pr-3"><strong>FUND CLUSTER: </strong>{{$l->cluster}}</td>
				</tr>
				<tr>
					<td colspan="5" class="p-3 pr-3"><strong>DIVISION: </strong>{{$l->division}}</td>
					<td colspan="2" class="p-3 pr-3"><strong>RESPONSIBILITY CENTER CODE: </strong>{{$l->respo_center}}</td>
				</tr>
				<tr>
					<td colspan="5" class="p-3 pr-3"><strong>PAPCODE: </strong>{{$l->papcode}}</td>
					<td colspan="2" class="p-3 pr-3"><strong>RIS NO.: </strong>{{$l->ris_num}}</td>
				</tr>
				...
				<tr>
					...
					<td align="center"><input style="text-align: center; width: 140px;" type="text" name="unit" id="unit" class="form-control"></td>
				...
				<tr>
					<td colspan="7" class="mb-5">
						<span class="d-flex float-right mb-2 mt-2" style="margin-top: 10px;">
							<button type="submit" class="btn btn-sm btn-primary mr-3 p-2" style="color: #fff"><span class="fa fa-floppy-o"></span> Save Request</button>
						</span>
					</td>
				</tr>
				{{--@foreach($data as $i)
				<tr>
					<td>{!!nl2br(str_replace(" ", " &nbsp;", $i->description))!!}</td>
					<td>{{$i->unit}}</td>
					<td>{{$i->quantity}}</td>
				</tr>
				@endforeach--}}
