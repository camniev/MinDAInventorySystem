...
<div class="content-wrapper" style="margin-left: 20px;">
    <section class="content-header">
    	<div class="content-header" style="color: #084B8A; font-family: 'Calibri'; font-size: 30px;"><Strong>Property Acknowledgement Receipt  &nbsp;&nbsp;&nbsp;&nbsp;<span id="qresult" style="color: #0B3861; font-style: bold"></span></Strong>
    		<span class="d-flex float-right mb-2 mt-3" style="margin-top: -3px;"><button onclick="export_excel();" class="btn btn-sm btn-success" style="color: #fff;"><span class="fa fa-file-excel-o" style="color: #fff;"></span> Export to Excel</button></span>
    	</div>
...
<table>
	...
	<th colspan="" class="d-flex" valign="center"><span class="mt-2" style="float: right;">Filter: </span>&nbsp;&nbsp;
							
		<select id="euser" name="euser" class="form-control mt-1 mb-1 mr-2" style="width:200px" onchange="searchEnduserChange();">
			@if($user->count()>0)
		...
		<tr>
			<td align="center" valign="center" rowspan="2" style="width: 80px; padding-left: 5px; border-right: 1px solid #A9D0F5; border: 1px solid #A9D0F5; background-color: #BDBDBD; color: #1C1C1C;"><strong>PAR No.</strong></td>
			...
			<td align="center" valign="tocenterp" rowspan="2" style="width: 130px; border-right: 1px solid #A9D0F5; border: 1px solid #A9D0F5; background-color: #BDBDBD; color: #1C1C1C;"><strong>Property Number</strong></td>
			<td align="center" valign="tocenterp" rowspan="2" style="width: 130px; border-right: 1px solid #A9D0F5; border: 1px solid #A9D0F5; background-color: #BDBDBD; color: #1C1C1C;"><strong>Acquisition Date</strong></td>
		...
		<tr>
			...
			<td class="p-3">{!!nl2br(str_replace(" ", " &nbsp;", $i->description))!!}</td>
			<td class="p-3">{{$i->prop_num}}</td>
			<td class="p-3">{{$i->date_receive}}</td>
		...
		@endif
		<tr>
			<td colspan="11" class="mb-5">
				<a href="{{ url('/property-acknowledgement-receipt') }}" class="btn btn-sm btn-success mt-1 mb-1 mr-1" style="color: #fff; float: right;"><span class="fa fa-list-ol" style="vertical-align: middle;"></span> List All</a>
...
<script>
	...
	function searchCY() {
	  ...
	  //alert(x);
	  window.location = "{{ url('/property-acknowledgement-receipt/list-by-fiscal-year') }}/" + x
	}
	...
	function searchEnduser() {
	  ...
	  if (confirm('Do you want to filter by FY and End-user?')) {
	    	window.location = "{{ url('/property-acknowledgement-receipt/list-by-fiscal-year-end-user') }}/" + x +"/" + y
		} else {
	    	window.location = "{{ url('/property-acknowledgement-receipt/list-by-end-user') }}/" + y
		}
		...
		if(id.length < 24){
			if(id.length >= 4 && id.length < 24){
				...
				if(isNaN(id)){
					...
					if(!isNaN(id2)){
						window.location = "{{ url('/export-excel-par/excel-output/cy-user-export') }}/"+id+"/"+id2;
					}else{
						window.location = "{{ url('/export-excel-par/excel-output/cy-user') }}/"+id;
					}
			...
			window.location = "{{ url('/export-excel-par/excel-output/list-all') }}";
		}
	}
</script>
@endsection