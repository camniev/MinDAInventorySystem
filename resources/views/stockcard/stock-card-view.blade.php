...
<table border="1px #fff solid;" style="table-layout: fixed;">
	<tr>
		...
		<td colspan="3" class="pl-3">{{$items->entity_name}}</td>
		...
		<td colspan="2"  class="pl-3">{{$items->cluster}}</td>
		<input type="hidden" name="inspection_id" value="">
		<input type="hidden" name="stockcard_id" value="">
	...
	<tr>
		...
		<td colspan="3" class="pl-3">{{$items->item}}</td>
		...
		<td colspan="2" class="pl-3">{{$items->stock_number}}</td>
	</tr>
	<tr>
		...
		<td colspan="3" class="pl-3">{{$items->description}}</td>
		...
		<td colspan="2" class="pl-3">{{$items->re_order}}</td>
	</tr>
	<tr>
		...
		<td colspan="6" class="pl-3">{{$items->unit}}</td>
	</tr>
	<tr>
	...
	<tr>
		<td align="center" valign="top" class=" p-3">{!!wordwrap(\Carbon\Carbon::parse($items->date_receive)->format('F j, Y'),10,"<br>\n")!!}</td>
		<td  class=" p-3" align="center"  valign="center">Physical Count</td>
		@if($items->physical_count < 0)
			<td  class=" p-3" align="center"  valign="center">{{$items->quantity}}</td>
			<td  class=" p-3" align="center"  valign="center"></td>
			<td  class=" p-3" align="center"  valign="center"></td>
			<td  class=" p-3" align="center"  valign="center">{{$items->quantity}}</td>
			<td  class=" p-3" align="center"  valign="center">{{$items->consume_days}}</td>
		@else
			<td  class=" p-3" align="center"  valign="center">{{$items->physical_count}}</td>
			<td  class=" p-3" align="center"  valign="center"></td>
			<td  class=" p-3" align="center"  valign="center"></td>
			<td  class=" p-3" align="center"  valign="center">{{$items->physical_count}}</td>
			<td  class=" p-3" align="center"  valign="center">{{$items->consume_days}}</td>
	...
	@foreach ($datas as $i)
		<tr>
				...
				@if($i->type=='iar')
					<td align="center" valign="top" class=" p-3">{!!wordwrap(\Carbon\Carbon::parse($i->created_at)->format('F j, Y'),10,"<br>\n")!!}</td>
...
<div>
	...
	<div class="d-flex float-right mt-3 mb-5">
		<a onclick="location = '{{ url('/stock-card') }}';" class="btn btn-sm btn-primary pl-3 pr-3 mr-2 mb-2" style="color: #fff"><span class="fa fa-chevron-left" style="vertical-align: middle;" ></span> Back</a></div>
</div>
@endif
