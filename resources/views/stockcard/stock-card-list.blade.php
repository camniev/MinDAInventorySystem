			...
			<tr>
				...
				@endif
					...
					<td style="width: 50px;">
						...
						{{--
							<a href="{{ url('/stock-card-lists/tag-accepted-item') }}/{{ $i->requesting_department }}/{{ $i->stock_id }}/{{ $i->inspection_id }}" style="text-decoration: none; margin-right: 10%;"><img src="{{ url('/images/new_button_uijk787hkjnkjhb6t5r7yugsd87f859346_960_7202.png') }}" width="45px" height="20px" style="margin-left: 5px;" title="New"></a>
										
							<a href="{{ url('/stock-card-lists/tagging-update-entry') }}/{{ $i->description }}/{{$i->id}}" style="text-decoration: none; margin-right: 10%;"><span class="fa fa-pencil-square-o"  style="margin-left: 5px;" title="Update"></span> Update</a>
						</div>

										
							<a href="{{ url('/stock-card-lists/tagging-update-entry-details') }}/{{$i->item}}" class="btn btn-sm btn-primary mt-2 mb-2" style="text-decoration: none; margin-right: 10%; color: #fff"><span class="fa fa-pencil-square-o"  style="margin-left: 5px;" title="Update"></span><br/>Update</a>
						--}}
						</div>

					<td colspan="2">
						<div class="row justify-content-center d-flex">
							{{--<a href = "{{ url('/stock-code/view-stock-list') }}/{{$i->stock_number}}" class="btn btn-sm btn-success ml-2 mr-2 mt-2 mb-2" style="text-decoration: none; margin-right: 10%; color: #fff" title="View Details"><span class="fa fa-eye" ></span><br/>View</a>--}}
...
<div class="modal fade" id="date-range" tabindex="-1" role="dialog"aria-labelledby="edit-modal-label" aria-hidden="true">
  <div class="modal-dialog  modal-lg" style="min-width: auto; max-width: 50%"  role="document">
    <div class="modal-content">
      <div class="modal-header"><span style="font-size: 24px; color: #FF4000;"><strong>Stock Card View</strong></span>
	...
	$(document).ready(function() {
		$(document).on("click", ".go_btn", function() {
			...
			if(o){
				...
				//alert(begin);
				url="{{ url('/stock-card/view-stock-list') }}/"+stock+"/"+begin+"/"+end;
