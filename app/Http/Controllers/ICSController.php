...
class ICSController extends Controller
{
    ...
    public function index()
    {
    	...
    	return view('ics.ics-list-view',compact('data','cy','user','reorderdata'));
    }
    ...
    public function fiscal_list($fy)
    {
        ...
        $data = DB::table('summaries')
                ->where(['summaries.type'=>'ris','summaries.category'=>'StockCard','summaries.cy'=>$fy])
                ->orderBy('summaries.created_at','desc')
                //->groupBy('summaries.requested_by')
                ->paginate(10)
                ->onEachSide(1);
    ...
    public function end_user($user)
    {
        ...
        $data = DB::table('summaries')
                ->where(['summaries.type'=>'ris','summaries.category'=>'StockCard','summaries.requested_by'=>$user])
                ->orderBy('summaries.created_at','desc')
                //->groupBy('summaries.requested_by')
                ->paginate(10)
                ->onEachSide(1);
    ...
    public function fiscal_end_user($fy,$user)
    {
        ...
        $data = DB::table('summaries')
                ->where(['summaries.type'=>'ris','summaries.category'=>'StockCard','summaries.requested_by'=>$user,'summaries.cy'=>$fy])
                ->orderBy('summaries.created_at','desc')
                //->groupBy('summaries.requested_by')
                ->paginate(10)
                ->onEachSide(1);
